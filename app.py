...
def _get_query_value(name, default=None):
    """兼容不同 Streamlit 版本的查询参数读取。"""
    try:
...
def _clear_query_values():
    """兼容不同 Streamlit 版本的查询参数清空。"""
    try:
...
def _active_page():
    """获取当前页面名称。"""
    pages = ["患者管理", "趋势分析", "导出与备份", "系统设置"]
    page = _get_query_value("page", "患者管理")
    return page if page in pages else "患者管理"


def _nav_link(label, page, active_page, icon):
    """生成固定左侧导航链接。"""
    active = "active" if page == active_page else ""
    href = "?page=" + quote(page)
    return f'<a class="wc-nav-item {active}" href="{href}"><span class="wc-nav-icon">{icon}</span><span>{label}</span></a>'


def render_fixed_left_nav(active_page):
    """绘制真正固定在页面左侧的导航栏，不再依赖 Streamlit 原生 sidebar。"""
    user = st.session_state.get("user") or {}
    display_name = user.get("display_name", "苏医生")

    nav_html = "\n".join([
        _nav_link("患者管理", "患者管理", active_page, "👥"),
        _nav_link("趋势分析", "趋势分析", active_page, "📈"),
        _nav_link("导出与备份", "导出与备份", active_page, "📦"),
        _nav_link("系统设置", "系统设置", active_page, "⚙️"),
    ])
    ...
    st.markdown(
        ...
        .wc-fixed-sidebar{{
            ...
            width: 246px;
            z-index: 999999;
            background: linear-gradient(180deg, #f8fffd 0%, #eef8f5 48%, #ffffff 100%);
            border: 1px solid #d9eee7;
            border-radius: 24px;
            box-shadow: 0 18px 45px rgba(31,50,74,.10);
            padding: 16px 14px;
            display: flex;
        ...
        .wc-brand-box{{
            ...
            border: 1px solid rgba(217,238,231,.95);
            border-radius: 20px;
        ...
        .wc-nav-item.active{{
            background: linear-gradient(135deg, #e8f4ff 0%, #eafaf5 100%);
            border-color: #cfe8ff;
            color: #0b5cab !important;
        ...
        .wc-user-avatar-fixed{{
            ...
            background:linear-gradient(135deg,#e6f7f4,#eaf3ff);
            color:#0f766e;
            font-weight:950;
        }}
        ...
        <div class="wc-fixed-sidebar">
            <div class="wc-brand-box">
                <img src="data:image/jpeg;base64,{BRAND_IMAGE_BASE64}">
                <div class="wc-system-title">减重门诊管理系统</div>
                <div class="wc-system-subtitle">逢安堂 · 在线版</div>
            </div>
            ...
            <div class="wc-sidebar-spacer"></div>

            <div class="wc-user-card-fixed">
                <div class="wc-user-avatar-fixed">苏</div>
                <div>
                    <div class="wc-user-name-fixed">{display_name}</div>
                    <div class="wc-user-role-fixed">门诊医生</div>
                </div>
            </div>
            <a class="wc-logout-link" href="?logout=1">退出登录</a>
        </div>
        """,
        unsafe_allow_html=True
    )
